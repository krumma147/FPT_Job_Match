...
const useAuth = () => {
  ...
  const login = async (username, password) => {
    try {
      ...
      if (response.data.message === 'OTP has been sent to your email. Please verify it.') {
        return true;
      }
  ...
  const confirmOTP = async (username, otp) => {
    try {
      const response = await axios.post('https://localhost:7282/api/Auth/Login-2FA-Email', {
        UserName: username,
        otp: otp
      });
  ...
  //register
  const signup = async (name, email, phone, password, confirmPassword, role) => {
    if (
      name === "" ||
      email === "" ||
      phone === "" ||
    ...
    try {
      const response = await axios.post('https://localhost:7282/api/Auth/Register', user);
      Swal.fire('Successful', response.data, 'success');
    } catch (error) {
      Swal.fire('Error', error.response.data.message, 'error');
    }
